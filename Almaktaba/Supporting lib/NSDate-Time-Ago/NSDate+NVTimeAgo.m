...
//    [formatter setTimeZone:[NSTimeZone localTimeZone]];
    NSDate *date = [formatter dateFromString:[mysqlDatetime stringByReplacingOccurrencesOfString:@"T" withString:@" "]];
...
{    
    ...
    if(secondsSince < 0)
        return _static_In_Future;
        
    
    // < 1 minute = "Just now"
    if(secondsSince < MINUTE)
        return _static_justnow;
...
- (NSString *)formatMinutesAgo:(NSTimeInterval)secondsSince
{
    ...
    //Handle Plural
    if(minutesSince == 1)
        return _static_1minute_ago;
    else
        return [NSString stringWithFormat:@"%d %@", minutesSince,_static_minute_ago];
}
...
- (NSString *)formatAsToday:(NSTimeInterval)secondsSince
{
    ...
    //Handle Plural
    if(hoursSince == 1)
        return _static_1Hour_ago;
    else
        return [NSString stringWithFormat:@"%d %@", hoursSince,_static_Hours_ago];
}
...
- (NSString *)formatAsYesterday
{
    ...
    return [NSString stringWithFormat:@"%@ %@",_static_yesterday_at, [dateFormatter stringFromDate:self]];
}
